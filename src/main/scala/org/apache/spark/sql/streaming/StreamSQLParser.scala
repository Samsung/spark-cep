 ...
 */
private[streaming]
class StreamSQLParser(streamSqlConnector: StreamSQLContext)
  extends AbstractSparkSQLParser with DataTypeParser {
  ...
  protected lazy val durationType: Parser[Duration] =
    ( (stringLit | numericLit) <~ MILLISECONDS ^^ { case s => Milliseconds(s.toInt) }
      | (stringLit | numericLit) <~ SECONDS ^^ { case s => Seconds(s.toInt) }
      | (stringLit | numericLit) <~ MINUTES ^^ { case s => Minutes(s.toInt) })
